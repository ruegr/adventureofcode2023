...
class Interpolator {
    ...
    interpolate() {
        ...
        for (let lineNo = this.lines.length - 2; lineNo >= 0; lineNo--) {
            const thisFirstElement = this.lines[lineNo].slice(0, 1) * 1;
            const nextLineFirstElement = this.lines[lineNo + 1].slice(0, 1) * 1;
            this.lines[lineNo].unshift(thisFirstElement - nextLineFirstElement);
        }
...
const result = interpolations.map(i => i.lines[0].slice(0,1)[0]).reduce((sum, val) => sum + val, 0);
